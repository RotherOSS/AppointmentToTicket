...
//             }
            if (Core.Config.Get('Action') === 'AgentAppointmentCalendarOverview') {
                // field has to be hidden
                if ( FieldInfo[1] == 0 ) {
                    // hide field div and label
                    Field.parent().hide();
                    $('label[for="' + FieldInfo[0] + '"]').hide();
                    Field.parent().addClass("oooACLHidden");
                ...
                // field has to be shown again
                else if ( Field.parent().hasClass("oooACLHidden") ) {
                    // show field and label
                    Field.parent().show();
                    $('label[for="' + FieldInfo[0] + '"]').show();
                    // TODO Validate this
                    // if it was hidden via autoselect before
                    Field.parent().show();
                    $("label[for='" + Field[0] + "']").show();
                    Field.parent().removeClass("oooACLHidden");
